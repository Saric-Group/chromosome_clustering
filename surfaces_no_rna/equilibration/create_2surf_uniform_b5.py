...
if((box_z-2*dz_free)/(bond_length*polymer_length)<1.):
	theta=arcsin((box_z-2*dz_free)/(bond_length*polymer_length))
else: 
	theta=pi/2.
...
natoms= nsurface_tot + npoly_tot*polymer_length 
nbonds= npoly_tot*polymer_length
nangles= npoly_tot*(polymer_length-1)

#A vector we will use to print the coordinates of the polymers
vec=array([0,cos(theta),sin(theta)])

with open('surfaces_Lz%.2f_npoly%d_Npoly%d_nspace%d.lammpsdata'%(box_z,npoly,polymer_length,nspace),'w') as fout:
